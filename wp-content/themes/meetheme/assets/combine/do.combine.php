<?php
	header('Server: Pluton');
	header('X-Powered-By: plutonthemes.com');
	$expires = 60 * 60 * 1;
	header("Pragma: public");
	header("Cache-Control: maxage=".$expires);
	header('Expires: '.gmdate('D, d M Y H:i:s', time() + $expires).' GMT');
	$pluton_denied = array('.exe', '.php', '.js', '.com', '.bat', '.html');
	if (isset($_REQUEST['scripts'])) {
		header('Content-type: text/javascript');
		checkRequest($_REQUEST['scripts']);
		$ext = '.js';
		foreach(explode(',', $_REQUEST['scripts']) as $file) {
			$file_path = dirname(dirname(__FILE__)).'/js/'.$file.$ext;
	...
	if (isset($_REQUEST['styles'])) {
		...
		foreach(explode(',', $_REQUEST['styles']) as $file) {
			$file_path = dirname(dirname(__FILE__)).'/css/'.$file.$ext;
			if (file_exists($file_path)) {
	...
	function checkRequest($req = null) {
		global $pluton_denied;
		foreach($pluton_denied as $extension) {
